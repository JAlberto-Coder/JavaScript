...
var cardsProjectComponent = {
    ...
    create: function (containerId, data, idField, contextMenuData = null, clickCallback = null) {
        ...
        data.forEach((item) => {
            ...
            card.classList.add(item.NombreEstatusProyecto.replace(/\s+/g, '-'));
            
            const cardContent = `
            <div class="card-special-logo">
                <img src="${item.ImagenCliente}" alt="">
            </div>
            <div class="card-special-title">
                <p>${item.NombreProyecto}</p>
            </div>
            <div class="card-special-entity">
                <p>${item.NombreEntidad}</p>
            </div>
            <div class="card-special-type">
                <p>${item.NombreTipoProyecto}</p>
            </div>
            <div class="card-special-responsability">
                <p title="Responsable"><i class="fa fa-user-circle"></i>&nbsp;${item.NombreResponsable}</p>
            </div>
            <div class="card-start-date">
                <p title="Inicio del proyecto">${item.InicioProyecto}</p>
            </div>
            <div class="card-end-date">
                <p title="Fin del proyecto">${item.FinProyecto}</p>
            </div>
            <div class="card-special-status">
                <p style="color: ${item.ColorEstatusProyecto}">${item.NombreEstatusProyecto}</p>
            </div>
            <div class="card-special-description-bk" style="background-image: linear-gradient(to top, ${item.ColorEstatusProyecto}, #FFFFFF);"></div>`;
